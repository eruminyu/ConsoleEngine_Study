...
namespace Craft
{
	...
	public:
		...
		// C++에서 인터페이스를 사용할 때 이렇게 구성함.
		virtual size_t GetType() const = 0;

		// 전달된 다른 타입 ID와 현재 객체(부모 포함)의 타입을 비교
	...
	std::shared_ptr<T> Cast(const std::shared_ptr<U>& object)
	{
		...
		// object의 실제 타입이 T(또는 T의 파생)인지 확인 한 후 형변환을 함(캐스팅)
...
{																			\
	/* 현재 계층에서 일단 비교한 후, 타입이 다르면 부모 계층까지 검색*/		\
